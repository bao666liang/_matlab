...
% ###############################################################
function [sys,x0,str,ts] = controlxl(t,x,u,flag)
switch flag,
case 0,
...
D0=[0.1;0.1;0.1];
CONG=diag([5*10^6,8*10^6,2*10^9]);

% Lambda=diag([1*10^(-7),1*10^(-7),1*10^(-11)]);%0
% D0=[0.1;0.1;0.1];
% CONG=diag([1*10^6,1*10^6,1*10^9]);

dDATAG=CONG*(XI*S2-Lambda*(DATAG-D0));
TAO=C*VV+D*VV+M*dVd-K2*S2-XI*DATAG;
